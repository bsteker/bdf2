package com.bstek.bdf2.dbconsole.view.datatab;
...
import com.bstek.bdf2.dbconsole.utils.ColumnTypeUtils;
 ...
 * @since 2.0
 */
@Controller("bdf2.dbconsole.DataTabInterceptor")
public class DataTabInterceptor {
	...
	public void onInit(DefaultEntityDataType dataTypeData) throws Exception {
		...
		DataTypeManager dataTypeManager = (DataTypeManager) ContextHolder.getBean("dorado.dataTypeManager");
		BasePropertyDef pd;
		if (columns != null) {
	...
	public void onInit(View view) throws Exception {
		...
		DataSet dataSetData = (DataSet) view.getComponent("dataSetData");
		...
		if (type.endsWith(MULTI_TYPE)) {
			...
			dataPilotData.setItemCodes("pages,|,pageSize,|");
