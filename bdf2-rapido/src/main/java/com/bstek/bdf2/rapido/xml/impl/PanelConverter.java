...
public class PanelConverter extends AbstractConverter {

	public Element convert(ComponentInfo component, RuleSet ruleSet,
			Element rootElement) throws Exception {
		String name=Panel.class.getSimpleName();
		XmlNode node=Panel.class.getAnnotation(XmlNode.class);
		if(node!=null && StringUtils.isNotEmpty(node.nodeName())){
			name=node.nodeName();
		}
	...
	public boolean support(ComponentInfo component) {
		return component.getClassName().equals(Panel.class.getName());
	}
}
